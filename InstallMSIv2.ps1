...
.DESCRIPTION
    This script downloads an MSI file from a specified URL and installs it silently with no restart.
    Additional command line parameters can be provided via CommandLineParams.

.PARAMETER MsiUrl
    The URL of the MSI file to download.

.PARAMETER CommandLineParams
    Command line parameters to pass to msiexec. Default is "/q /norestart" for silent install with no reboot.

.EXAMPLE
    .\InstallMSIv2.ps1 -MsiUrl "https://example.com/package.msi"

.EXAMPLE
    .\InstallMSIv2.ps1 -MsiUrl "https://example.com/package.msi" -CommandLineParams "/q /norestart INSTALLDIR=C:\CustomPath"
#>
...
param(
    ...
    [Parameter(Mandatory=$false, HelpMessage="Command line parameters for msiexec")]
    [string]$CommandLineParams = "/q /norestart"
)
...
try {
    ...
    if (-not [string]::IsNullOrWhiteSpace($CommandLineParams)) {
        ...
        Write-Log "Command line parameters: $CommandLineParams"
    }
